...
<div class="container mt-4">
    <h2 class="mb-4 form-cls">Daily Report</h2>
    <form method="GET" name="filter">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <select id="project_id" name="project_id" class="select-box form-control">
                            ...
                            <option value="{{ $value->project_detail->id }}" @if(!empty($project_id) && $project_id==$value->project_detail->id) selected @endif>{{ $value->project_detail->project_name }}</option>
                            @endforeach
                            @else 
                            @foreach($projects as $key => $value)
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-6 mb-3">
                ...
                <a href="{{ route('daily_report') }}" class="col-md-3"><button type="button" class="btn btn-danger mt-4"><i class="fa-solid fa-rotate-right"></i> Clear Search</button></a>
            </div>
    ...
    <table class="table table-bordered project-datatable" id="datatable">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ !empty($val->client_detail) ? $val->client_detail->name : '' }}</td>
                @if(\Auth::user()->role == 1)
                <td><a href="javascript:void(0)" class="view-btn" data-id={{$val->project_id}}>{{ !empty($val->project_detail) ? $val->project_detail->project_name : ''}}</a></td>
                @else
                <td>{{ !empty($val->project_detail) ? $val->project_detail->project_name : ''}}</td>
                @endif
                <td>{{ !empty($val->employee_detail) ? $val->employee_detail->name : '' }}</td>
                <td>{{ $val->task_name }}</td>
                <td>{{ minutes_to_hour($val->time_taken) }}</td>
            </tr>
            @endforeach
        </tbody>
        <tbody>
        <tr>
            ...
            <td></td>
            <td>Total Hours</td>
            <td>{{ minutes_to_hour($total_minutes_taken) }}</td>
        </tr>   
        </tbody>
    </table>
</div>
    ...
    $(document).on('click', '.view-btn', function() {
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                var work_e_date = moment(new Date(res.project_det.work_start_date)).format('DD-MM-YYYY');
